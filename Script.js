...
// and robust image fallback with support for data-alt-src cycling before placeholder.
(function () {
  // Utility
  ...
  // Image fallback with alt-src cycling
  (function setupImageFallbacks() {
    ...
    qsa("img").forEach((img) => {
      // initialize alt-src index
      if (!img.dataset.altIndex) img.dataset.altIndex = "0";

      const tryNextAlt = (imageElement) => {
        const altSrcs = (imageElement.dataset.altSrc || "").split(",").map(s => s.trim()).filter(Boolean);
        const currentIndex = parseInt(imageElement.dataset.altIndex || "0", 10);
        if (currentIndex < altSrcs.length) {
          const next = altSrcs[currentIndex];
          // increment counter for subsequent failures
          imageElement.dataset.altIndex = String(currentIndex + 1);
          // update src and clear srcset to avoid conflicting sources (optional)
          imageElement.src = next;
          // Also try updating srcset if you want to provide alt srcset variants:
          // imageElement.srcset = '';
          return true;
        }
        return false;
      ...
      // If the image already failed to load (naturalWidth === 0 and complete), attempt alt immediately
      ...
      // Attach an error listener to swap in the next alt or placeholder
      img.addEventListener("error", function onError() {
        // avoid infinite loop: if fallbackApplied set, do nothing
        if (img.dataset.fallbackApplied) return;
        // try next alt source if provided
        const usedAlt = tryNextAlt(img);
        if (!usedAlt) {
          // mark fallback applied and set placeholder
          img.dataset.fallbackApplied = "1";
          img.classList.add("img-fallback");
          img.src = placeholderSvg;
        } else {
          // If we tried an alt, re-attach this error listener for the next failure
          // (listener with once:true would have been removed, so ensure continued handling)
          // Nothing else to do; the new src may trigger load/error again.
        }
      }, { once: false });
    });
  })();
